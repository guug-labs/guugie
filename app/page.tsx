...
export default function GuugieFinalPage() {
  ...
  // ==================== DATA LOADER (FIX MINUS DI SINI) ====================
  const loadData = useCallback(async (uid: string) => {
    const { data: prof } = await supabase.from("profiles").select("quota").eq("id", uid).single();
    
    if (prof) {
      // LOGIC ANTI MINUS SAAT LOAD: Kalau poin < 0, paksa jadi 0 dan update DB
      if (prof.quota < 0) {
        setQuota(0);
        await supabase.from("profiles").update({ quota: 0 }).eq("id", uid);
      } else {
        setQuota(prof.quota);
      }
    } else {
      // Kalau user baru, kasih modal 25
  ...
  const toggleMic = () => {
    ...
    recognition.onerror = () => { setIsListening(false); showToast('error', 'Gagal mendeteksi suara.'); };
  ...
  // ==================== SEND MESSAGE (LOGIC ANTI JEBOL) ====================
  const handleSendMessage = async () => {
    const model = GUUGIE_MODELS[selectedKasta];
    const cost = model.points;
    const currentQuota = quota ?? 0; // Pastikan tidak null

    // 1. CEK SALDO (STRICT)
    if (currentQuota < cost) {
      return showToast('error', `Poin kurang! Sisa: ${currentQuota}, Butuh: ${cost}`);
    }
    ...
    try {
      ...
      if (data.content) {
        ...
        // 2. UPDATE POIN (LOGIC SAFETY MATH.MAX)
        if (cost > 0) {
          // Paksa minimal 0, gak boleh minus
          const safeNewQuota = Math.max(0, currentQuota - cost);
          
          setQuota(safeNewQuota); 
          // Update DB dengan nilai yang aman
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-white/[0.04]">
             ...
             <div className="flex-1 min-w-0">
               ...
               <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-[10px] font-bold text-white/60">{Math.max(0, quota ?? 0)} PTS</span>
               </div>
             </div>
      ...
      <main className="flex-1 flex flex-col relative w-full h-full bg-[#0a0a0a]">
        <header className="h-16 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/[0.04]">
          ...
          <div className="lg:hidden flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/[0.05]">
            ...
            <span className="text-xs font-bold">{Math.max(0, quota ?? 0)}</span>
          </div>
        </header>
            ...
            <div className="space-y-6 pt-6">
              {messages.map((m, i) => (
                <div key={i} className={`flex w-full ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[88%] lg:max-w-[85%] rounded-2xl p-4 text-[15px] leading-relaxed ${m.role === 'user' ? 'bg-[#262626] text-white rounded-tr-sm border border-white/5' : 'bg-transparent text-[#e5e5e5] px-0 lg:px-2'}`}>
                    {/* @ts-ignore */}
                    <ReactMarkdown className="markdown-body" remarkPlugins={[remarkGfm]}>{m.content as string}</ReactMarkdown>
                  </div>
                </div>
              ))}
              {isLoading && <div className="flex items-center gap-3 text-white/30 text-xs px-2 animate-pulse"><Loader2 className="animate-spin" size={14} /><span>{GUUGIE_MODELS[selectedKasta].loading}</span></div>}
              <div ref={chatEndRef} />
            </div>
          )}
        </div>
        ...
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a] to-transparent pt-10 pb-6 px-4 safe-area-bottom z-40">
          <div className="max-w-3xl mx-auto space-y-3">
            ...
            <div className="bg-[#1a1a1a] border border-white/[0.08] rounded-[26px] p-2 flex flex-col focus-within:border-white/20 transition-all shadow-2xl relative">
              ...
              <textarea ref={textareaRef} value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => { if(e.key === 'Enter' && !e.shiftKey && !isLoading) { e.preventDefault(); handleSendMessage(); } }} placeholder="Tanya Guugie..." className="w-full bg-transparent border-none focus:ring-0 text-[15px] text-white placeholder-white/30 px-4 py-2 resize-none no-scrollbar max-h-[160px] min-h-[44px]" rows={1} />
              
              <div className="flex items-center justify-between px-2 pb-1">
                <div className="flex items-center gap-1">
                  <button onClick={toggleMic} className={`p-2 rounded-full transition-colors ${isListening ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>{isListening ? <MicOff size={20} /> : <Mic size={20} />}</button>
                  <button onClick={() => fileInputRef.current?.click()} className="p-2 text-white/40 hover:text-white hover:bg-white/5 rounded-full transition-colors"><Paperclip size={20} /></button>
                </div>
                <input type="file" ref={fileInputRef} className="hidden" multiple onChange={handleFileUpload} />
                <button onClick={handleSendMessage} disabled={isLoading || (!inputText.trim() && pendingFiles.length === 0)} className={`p-2 rounded-full transition-all duration-200 flex items-center justify-center ${inputText.trim() || pendingFiles.length > 0 ? 'bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.2)] hover:scale-105' : 'bg-white/10 text-white/20 cursor-not-allowed'}`}>{isLoading ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}</button>
              </div>
            </div>
