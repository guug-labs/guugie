...
export default function GuugieHyperFinalPage() {
  ...
  const textAreaRef = useRef<HTMLTextAreaElement>(null); // REF UNTUK FIX AUTO-EXPAND
  ...
  // LOGIC FIX: AUTO-EXPAND TEXTAREA
  useEffect(() => {
  ...
  const handleSendMessage = async () => {
    // FIX BUG ENTER 2X: Loading guard
  ...
  const Modal = ({ title, children }: { title: string, children: React.ReactNode }) => (
    <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-[#0B101A]/95 backdrop-blur-2xl animate-in fade-in duration-300">
      <div className="bg-[#1E293B] border border-white/10 w-full max-w-2xl rounded-[40px] overflow-hidden shadow-2xl relative">
        <button onClick={() => setActiveModal(null)} className="absolute top-6 right-6 p-3 hover:bg-white/10 rounded-2xl transition-all text-slate-400 hover:text-white z-10"><X size={24}/></button>
        <div className="p-10 border-b border-white/5 bg-white/5">
          <h3 className="text-xs font-black uppercase tracking-[0.3em] text-blue-500">{title}</h3>
        </div>
        <div className="p-10 max-h-[60vh] overflow-y-auto text-sm text-slate-300 custom-scrollbar leading-relaxed">
          {children}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex h-screen bg-[#0B101A] text-slate-200 overflow-hidden font-sans">
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 z-[300] bg-[#0B101A] flex items-center justify-center p-4">
          ...
          <div className="w-full max-w-5xl text-center relative z-10 animate-in fade-in zoom-in duration-700">
            <h2 className="text-5xl lg:text-7xl font-black italic uppercase tracking-tighter mb-4 text-white drop-shadow-2xl">SIAPA ANDA?</h2>
            <p className="text-[10px] lg:text-xs font-black uppercase tracking-[0.3em] text-blue-500/60 mb-16">PILIHAN INI PERMANEN UNTUK AKUN ANDA</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-4">
              <button onClick={() => lockCategory('MAHASISWA')} className="group relative p-10 bg-[#1E293B]/40 border border-white/5 rounded-[45px] hover:border-blue-500/50 transition-all hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="w-20 h-20 bg-blue-600/10 rounded-3xl flex items-center justify-center mx-auto mb-8 group-hover:bg-blue-600 group-hover:text-white transition-all text-blue-500 shadow-xl"><GraduationCap size={40}/></div>
                <h3 className="font-black uppercase tracking-widest text-lg mb-3 text-white">Mahasiswa</h3>
                <p className="text-[10px] font-bold text-slate-500 uppercase leading-relaxed text-center">Riset & Sidang</p>
              </button>
              <button onClick={() => lockCategory('PELAJAR')} className="group relative p-10 bg-[#1E293B]/40 border border-white/5 rounded-[45px] hover:border-orange-500/50 transition-all hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] overflow-hidden">
                ...
                <div className="w-20 h-20 bg-orange-600/10 rounded-3xl flex items-center justify-center mx-auto mb-8 group-hover:bg-orange-600 group-hover:text-white transition-all text-orange-500 shadow-xl"><School size={40}/></div>
                <h3 className="font-black uppercase tracking-widest text-lg mb-3 text-white text-center">Pelajar</h3>
                <p className="text-[10px] font-bold text-slate-500 uppercase leading-relaxed text-center">SD-SMA & SMK</p>
              </button>
              <button onClick={() => lockCategory('PROFESIONAL')} className="group relative p-10 bg-[#1E293B]/40 border border-white/5 rounded-[45px] hover:border-emerald-500/50 transition-all hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] overflow-hidden">
                ...
                <div className="w-20 h-20 bg-emerald-600/10 rounded-3xl flex items-center justify-center mx-auto mb-8 group-hover:bg-emerald-600 group-hover:text-white transition-all text-emerald-500 shadow-xl"><Briefcase size={40}/></div>
                <h3 className="font-black uppercase tracking-widest text-lg mb-3 text-white text-center">Profesional</h3>
      ...
      {/* --- 🛠️ MODAL CONTENT --- */}
      {activeModal === 'library' && <Modal title="Library Riset"><div className="space-y-4"><p className="font-bold text-white">Penyimpanan Terpusat</p><p>Kelola semua dokumen riset Anda di sini. Fitur ini memungkinkan Anda menyimpan hasil analisis penting dan sitasi dalam satu tempat yang aman.</p></div></Modal>}
      {activeModal === 'tos' && <Modal title="Terms of Service"><div className="space-y-4"><p className="font-bold text-white">Ketentuan Penggunaan</p><p>Guugie dirancang untuk membantu produktivitas akademik. Dilarang keras menggunakan AI ini untuk tindakan plagiarisme atau manipulasi data ilegal yang merugikan integritas pendidikan.</p></div></Modal>}
      {activeModal === 'privacy' && <Modal title="Privacy Policy"><div className="space-y-4"><p className="font-bold text-white">Keamanan Data</p><p>Privasi Anda adalah prioritas kami. Semua file dan chat dienkripsi dan tidak digunakan untuk pelatihan model AI pihak ketiga tanpa izin eksplisit dari Anda.</p></div></Modal>}
      {activeModal === 'feedback' && <Modal title="Kritik & Saran"><div className="space-y-6"><p>Kami sangat menghargai masukan Anda untuk perkembangan Guugie agar lebih baik lagi.</p><div className="bg-white/5 p-6 rounded-2xl border border-white/10"><p className="text-[10px] font-black uppercase text-blue-500 mb-2">Hubungi Kami Melalui Email</p><p className="text-lg font-bold">guuglabs@gmail.com</p></div><p className="text-xs italic text-slate-500">Kirimkan ide fitur baru, laporan bug, atau sekadar sapaan melalui alamat email di atas.</p></div></Modal>}

      <aside className={`fixed lg:relative z-50 h-full transition-all duration-500 bg-[#0F172A] border-r border-white/5 flex flex-col ${isSidebarOpen ? "w-72 shadow-2xl" : "w-0 overflow-hidden"}`}>
      ...
      {/* FIX TAMPILAN: flex flex-col h-full overflow-hidden agar area chat bisa discroll */}
      <main className="flex-1 flex flex-col relative min-w-0 h-full overflow-hidden">
        <header className="shrink-0 flex items-center justify-between p-4 lg:p-6 bg-[#0B101A]/80 backdrop-blur-xl border-b border-white/5 z-40">
          ...
          <div className="flex items-center gap-4 relative">
            <div className="bg-orange-500/10 border border-orange-500/20 px-4 py-1.5 rounded-full"><span className="text-[10px] font-black text-orange-500 uppercase tracking-widest">Poin: {quota}</span></div>
        ...
        {/* AREA CHAT: flex-1 overflow-y-auto agar tidak tertutup teks panjang */}
        <div className="flex-1 overflow-y-auto p-4 lg:p-8 custom-scrollbar scroll-smooth">
          <div className="max-w-4xl mx-auto space-y-10 pb-10 flex flex-col items-center">
            {messages.length === 0 ? (
              <div className="mt-20 text-center max-w-xl animate-in fade-in slide-in-from-bottom-8 duration-1000">
              ...
              <div className="w-full space-y-8 pb-20">
                {messages.map((m, i) => (
                  <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-4`}>
                    <div className={`max-w-[95%] lg:max-w-[85%] p-5 lg:p-7 rounded-[28px] lg:rounded-[36px] text-[13px] lg:text-[15px] border shadow-2xl ${m.role === 'user' ? 'bg-[#1E293B] border-white/5 rounded-tr-none' : 'bg-blue-600/5 border-blue-500/10 rounded-tl-none'}`}>
                      <div className="prose prose-invert prose-sm lg:prose-base max-w-none text-slate-300 leading-relaxed"><ReactMarkdown remarkPlugins={[remarkGfm]}>{m.content}</ReactMarkdown></div>
                    </div>
                  </div>
                ))}
                {isLoading && (
        ...
        {/* INPUT BOX TETAP DI BAWAH (shrink-0) */}
        <div className="shrink-0 p-4 lg:p-8 bg-gradient-to-t from-[#0B101A] via-[#0B101A] to-transparent">
          <div className="max-w-4xl mx-auto relative">
            ...
            <div className="absolute -top-12 left-2 z-50">
              <button onClick={() => setIsModeMenuOpen(!isModeMenuOpen)} className="flex items-center gap-3 px-4 py-2 bg-[#1E293B] border border-white/10 rounded-xl text-[9px] font-black uppercase shadow-xl hover:border-blue-500/30 transition-all">
            ...
            {/* FIX: Textarea dengan ref={textAreaRef} dan auto-expand logic */}
            <div className="bg-[#1E293B] border border-white/10 rounded-[28px] lg:rounded-[38px] p-2.5 lg:p-3.5 shadow-2xl focus-within:ring-2 focus-within:ring-blue-500/20 transition-all flex items-end gap-1 lg:gap-3 backdrop-blur-sm">
              <button onClick={() => fileInputRef.current?.click()} disabled={isUploading || !!pendingFile} className="p-3 lg:p-4 text-slate-500 hover:bg-white/5 rounded-2xl transition-all active:scale-90">{isUploading ? <Loader2 size={20} className="animate-spin text-blue-500" /> : <Paperclip size={20} />}</button>
              ...
              <textarea 
                ...
                placeholder={`Berdiskusi sebagai ${userCategory}...`} 
                className="flex-1 bg-transparent border-none outline-none p-3 lg:p-4 text-[14px] lg:text-[16px] font-medium resize-none max-h-48 custom-scrollbar placeholder:text-slate-600" 
              />
              <button onClick={startListening} className={`p-3 lg:p-4 rounded-2xl transition-all duration-300 ${isListening ? 'bg-red-500 text-white animate-pulse shadow-[0_0_25px_rgba(239,68,68,0.5)]' : 'text-slate-500 hover:bg-white/5 hover:text-red-500 hover:shadow-[0_0_15px_rgba(239,68,68,0.2)]'}`}><Mic size={20} /></button>
              <button onClick={handleSendMessage} disabled={isLoading} className="p-3 lg:p-5 bg-blue-600 text-white rounded-[20px] lg:rounded-[24px] flex items-center justify-center min-w-[55px] lg:min-w-[70px] transition-all active:scale-95 shadow-xl hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">{isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send size={22} />}</button>
            </div>

            {/* --- 🛡️ FOOTER SYSTEM TETAP DI SINI --- */}
            <footer className="mt-10 flex flex-wrap justify-center items-center gap-x-10 gap-y-5 text-[9px] lg:text-[11px] font-black uppercase tracking-[0.3em] text-white/30">
              <button onClick={() => setActiveModal('library')} className="flex items-center gap-2.5 hover:text-blue-500 transition-all group"><BookOpen size={16} className="group-hover:scale-110 transition-transform"/> Library</button>
              <button onClick={() => setActiveModal('tos')} className="flex items-center gap-2.5 hover:text-blue-500 transition-all group"><ShieldCheck size={16} className="group-hover:scale-110 transition-transform"/> Terms</button>
              <button onClick={() => setActiveModal('privacy')} className="flex items-center gap-2.5 hover:text-blue-500 transition-all group"><Shield size={16} className="group-hover:scale-110 transition-transform"/> Privacy</button>
              <div className="h-4 w-[1px] bg-white/10 hidden md:block" />
              <button onClick={() => setActiveModal('feedback')} className="flex items-center gap-2.5 hover:text-blue-400 transition-all text-blue-500/50 group font-black"><MessageSquare size={16} className="group-hover:scale-110 transition-transform"/> Kritik & Saran</button>
              <p className="w-full text-center mt-6 text-white/10 select-none tracking-[0.5em] font-black">© 2026 GUUG LABS</p>
            </footer>
          </div>
        </div>
      </main>
